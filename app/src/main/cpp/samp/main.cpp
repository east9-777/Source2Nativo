...
jint JNI_OnLoad(JavaVM* vm, void* reserved)
{
	...
	JNIEnv* env = nullptr;
	bool attachedForRegistration = false;
	if (vm->GetEnv(reinterpret_cast<void**>(&env), JNI_VERSION_1_6) != JNI_OK || env == nullptr) {
		if (vm->AttachCurrentThread(&env, nullptr) != JNI_OK) {
			FLog("[CEF] Failed to attach thread to register CefBrowser class");
			return JNI_VERSION_1_6;
		}
		attachedForRegistration = true;
	}

	jclass localCefClass = env->FindClass("com/raiferoleplay/game/cef/CefBrowser");
	if (!localCefClass) {
		FLog("[CEF] Unable to locate com/raiferoleplay/game/cef/CefBrowser");
	} else {
		if (CCefBrowser::clazz) {
			env->DeleteGlobalRef(CCefBrowser::clazz);
		}
		CCefBrowser::clazz = static_cast<jclass>(env->NewGlobalRef(localCefClass));
		env->DeleteLocalRef(localCefClass);
		if (!CCefBrowser::clazz) {
			FLog("[CEF] Failed to promote CefBrowser class to global reference");
		}
	}

	if (attachedForRegistration) {
