...
void CNetGame::Packet_CefRpc(Packet *p) {
	if (!p) {
		return;
	}

	CCefBrowser::ReceivePacket(p);
}
...
void CNetGame::InitGameLogic()
{
	...
	const int kDebugBrowserId = 1;
	const int cefScreenW = 2712;
	const int cefScreenH = 1220;
	const int widgetW = 320;
	const int widgetH = 140;
	const int widgetX = (cefScreenW - widgetW) / 2;
	const int widgetY = cefScreenH - widgetH;

	FLog("[CEF] Requesting debug overlay 'cef-demo' at %dx%d (%d,%d)", widgetW, widgetH, widgetX, widgetY);
	CCefBrowser::DestroyBrowser(kDebugBrowserId);
	CCefBrowser::CreateBrowser(
		/*browserId*/ kDebugBrowserId,
		/*x*/ widgetX,
		/*y*/ widgetY,
		/*width*/ widgetW,
		/*height*/ widgetH,
		"resource://assets/cef-demo/index.html",
		/*visible*/ true,
		/*bgAlpha*/ 0,
		/*offscreen*/ false);
	CCefBrowser::SetInteractive(kDebugBrowserId, false);
}
